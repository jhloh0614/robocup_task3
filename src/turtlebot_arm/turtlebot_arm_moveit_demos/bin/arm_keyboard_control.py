...
class arm_keyboard():
    ...
    def keyboard_control(self):
        while(1):
            ...
            if key == 't':
                self.joint1_curr += 1
                self.joint1.publish(0.01*self.joint1_curr)
            if key == 'g':
                self.joint1_curr -= 1
                self.joint1.publish(0.01*self.joint1_curr)
            if key == 'y':
                self.joint2_curr += 1
                self.joint2.publish(0.01*self.joint2_curr)
            if key == 'h':
                self.joint2_curr -= 1
                self.joint2.publish(0.01*self.joint2_curr)
            if key == 'u':
                self.joint3_curr += 1
                self.joint3.publish(0.01*self.joint3_curr)
            if key == 'j':
                self.joint3_curr -= 1
                self.joint3.publish(0.01*self.joint3_curr)
            if key == 'i':
                self.joint4_curr += 1
                self.joint4.publish(0.01*self.joint4_curr)
            if key == 'k':
                self.joint4_curr -= 1
                self.joint4.publish(0.01*self.joint4_curr)
            if key == 'o':
                self.joint5_curr += 1
                self.joint5.publish(0.01*self.joint5_curr)
            if key == 'l':
                self.joint5_curr -=1 
                self.joint5.publish(0.01*self.joint5_curr)
